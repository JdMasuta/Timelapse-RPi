...
// Hide video progress and reset UI
function hideVideoProgress() {
  ...
  if (generateBtn) {
    ...
    generateBtn.textContent = "🎬 Generate Time-lapse Video";
  }

  if (cancelBtn) {
    cancelBtn.style.display = "none";
    cancelBtn.disabled = false;
    cancelBtn.textContent = "⏹️ Cancel Generation";
  }
}

// Update video progress
function updateVideoProgress(progress, message) {
  const progressFill = document.getElementById("videoProgressFill");
  const statusText = document.getElementById("videoStatus");
      ...
      <div class="file-item video-item">
        <div class="file-info">
          ...
          <div class="file-details">
            <span class="file-size">${video.sizeFormatted}</span>
            <span class="file-date">${video.createdFormatted}</span>
          </div>
        </div>
        <div class="file-actions">
          <button class="btn btn-small btn-primary" onclick="previewVideo('${video.filename}')" title="Preview">
            👁️ Preview
          </button>
          <button class="btn btn-small btn-success" onclick="downloadVideo('${video.filename}')" title="Download">
            📥 Download
          </button>
          <button class="btn btn-small btn-danger" onclick="deleteVideo('${video.filename}')" title="Delete">
            🗑️ Delete
          </button>
        </div>
      </div>
...
function updateVideoStatus(status) {
  ...
  const processingProgress = document.getElementById("videoProcessingProgress");
  ...
  if (statusDisplay && status) {
    if (status.isProcessing) {
      ...
      if (processingStatus) {
        ...
        processingStatus.className = "value processing";
      }

      if (processingProgress) {
        const duration = status.currentJob
          ? Math.round((Date.now() - status.currentJob.startTime) / 1000)
          : 0;
        processingProgress.textContent = `${duration}s`;
      }
    } else {
      if (processingStatus) {
        processingStatus.textContent = "Ready";
        processingStatus.className = "value ready";
      }
...
function showNotification(message, type = "info") {
  ...
  const hideDelay =
    type === "success" && message.toLowerCase().includes("config")
      ? 4000
      : 3000;
  setTimeout(() => {
    ...
    captureInterval.addEventListener("input", function () {
      ...
      if (value < 1 || value > 3600) {
        this.setCustomValidity(
          "Capture interval must be between 1 and 3600 seconds"
        );
      } else {
        this.setCustomValidity("");
      }
...
function initializeTooltips() {
  const tooltips = [
    {
      id: "videoCodec",
      text: "H.264: Most compatible, H.265: Better compression, VP9: Open source",
    },
    {
      id: "videoBitrate",
      text: "Higher bitrate = better quality but larger file sizes",
    },
    {
      id: "enableHardwareAcceleration",
      text: "Uses GPU for faster video encoding if available",
    },
    {
      id: "debugMode",
      text: "Enables detailed logging for troubleshooting issues",
    },
    {
      id: "mockCamera",
      text: "Simulates camera for testing without real hardware",
    },
  ];
...
function showConfigurationSummary(config) {
  ...
  summaryDiv.innerHTML = `
        ...
        <div class="config-summary-grid">
            ...
            <div class="config-summary-item">
                <span class="config-summary-value">${
                  config.videoCodec?.toUpperCase() || "H264"
                }</span>
...
function showConfigurationReminder() {
  ...
  if (changedElements.length > 0) {
    showNotification(
      `${changedElements.length} setting(s) changed. Don't forget to save!`,
      "info"
    );
  }
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  console.log(
    "Enhanced UI initialized with comprehensive configuration support"
  );
});
