...
class ConfigService {
  ...
  async loadConfig() {
    try {
      ...
      const config = {
        ...
        // MJPG-Streamer Configuration
        mjpgStreamerPort:
          parseInt(process.env.MJPG_STREAMER_PORT) ||
          this.defaultConfig.MJPG_STREAMER_PORT,
        mjpgStreamerPath:
          process.env.MJPG_STREAMER_PATH ||
          this.defaultConfig.MJPG_STREAMER_PATH,
        mjpgStreamerWww:
          process.env.MJPG_STREAMER_WWW || this.defaultConfig.MJPG_STREAMER_WWW,

        // Timelapse Settings (main config used by web interface)
        captureInterval:
          this.parseNumber(process.env.CAPTURE_INTERVAL, 1, 3600) ||
          this.defaultConfig.CAPTURE_INTERVAL,
        outputDir: process.env.OUTPUT_DIR || this.defaultConfig.OUTPUT_DIR,
        imageQuality:
          this.validateOption(process.env.IMAGE_QUALITY, "imageQuality") ||
          this.defaultConfig.IMAGE_QUALITY,
        maxStorageGb:
          parseInt(process.env.MAX_STORAGE_GB) ||
          this.defaultConfig.MAX_STORAGE_GB,

        // Camera Stream Settings
        streamFps:
          this.validateOption(parseInt(process.env.STREAM_FPS), "streamFps") ||
          this.defaultConfig.STREAM_FPS,
        streamWidth:
          parseInt(process.env.STREAM_WIDTH) || this.defaultConfig.STREAM_WIDTH,
        streamHeight:
          parseInt(process.env.STREAM_HEIGHT) ||
          this.defaultConfig.STREAM_HEIGHT,
        streamQuality:
          this.validateOption(process.env.STREAM_QUALITY, "streamQuality") ||
          this.mapStreamQuality(
            process.env.STREAM_WIDTH,
            process.env.STREAM_HEIGHT
          ),

        // Daily Schedule Settings
        scheduleEnabled:
          this.parseBool(process.env.SCHEDULE_ENABLED) ??
          this.defaultConfig.SCHEDULE_ENABLED,
        startTime:
          this.validateTime(process.env.SCHEDULE_START_TIME) ||
          this.defaultConfig.SCHEDULE_START_TIME,
        stopTime:
          this.validateTime(process.env.SCHEDULE_STOP_TIME) ||
          this.defaultConfig.SCHEDULE_STOP_TIME,

        // Video Generation Settings
        videoFps:
          this.validateOption(parseInt(process.env.VIDEO_FPS), "videoFps") ||
          this.defaultConfig.VIDEO_FPS,
        videoQuality:
          this.validateOption(process.env.VIDEO_QUALITY, "videoQuality") ||
          this.defaultConfig.VIDEO_QUALITY,
        videoCodec:
          this.validateOption(process.env.VIDEO_CODEC, "videoCodec") ||
          this.defaultConfig.VIDEO_CODEC,
        videoBitrate:
          this.validateOption(process.env.VIDEO_BITRATE, "videoBitrate") ||
          this.defaultConfig.VIDEO_BITRATE,

        // Advanced Camera Settings
        cameraType:
          this.validateOption(process.env.CAMERA_TYPE, "cameraType") ||
          this.defaultConfig.CAMERA_TYPE,
        cameraDevice:
          process.env.CAMERA_DEVICE || this.defaultConfig.CAMERA_DEVICE,
        resolutionWidth:
          parseInt(process.env.RESOLUTION_WIDTH) ||
          this.defaultConfig.RESOLUTION_WIDTH,
        resolutionHeight:
          parseInt(process.env.RESOLUTION_HEIGHT) ||
          this.defaultConfig.RESOLUTION_HEIGHT,
        rotation:
          this.validateOption(parseInt(process.env.ROTATION), "rotation") ||
          this.defaultConfig.ROTATION,
        flipHorizontal:
          this.parseBool(process.env.FLIP_HORIZONTAL) ??
          this.defaultConfig.FLIP_HORIZONTAL,
        flipVertical:
          this.parseBool(process.env.FLIP_VERTICAL) ??
          this.defaultConfig.FLIP_VERTICAL,

        // Storage Management
        autoCleanup:
          this.parseBool(process.env.AUTO_CLEANUP) ??
          this.defaultConfig.AUTO_CLEANUP,
        maxImages:
          parseInt(process.env.MAX_IMAGES) || this.defaultConfig.MAX_IMAGES,
        cleanupOlderThanDays:
          parseInt(process.env.CLEANUP_OLDER_THAN_DAYS) ||
          this.defaultConfig.CLEANUP_OLDER_THAN_DAYS,
        autoGenerateVideo:
          this.parseBool(process.env.AUTO_GENERATE_VIDEO) ??
          this.defaultConfig.AUTO_GENERATE_VIDEO,

        // Performance & Resource Settings
        enableHardwareAcceleration:
          this.parseBool(process.env.ENABLE_HARDWARE_ACCELERATION) ??
          this.defaultConfig.ENABLE_HARDWARE_ACCELERATION,

        // Logging Configuration
        logLevel:
          this.validateOption(process.env.LOG_LEVEL, "logLevel") ||
          this.defaultConfig.LOG_LEVEL,
        logFile: process.env.LOG_FILE || this.defaultConfig.LOG_FILE,

        // Development & Debug Settings
        debugMode:
          this.parseBool(process.env.DEBUG_MODE) ??
          this.defaultConfig.DEBUG_MODE,
        mockCamera:
          this.parseBool(process.env.MOCK_CAMERA) ??
          this.defaultConfig.MOCK_CAMERA,
      };
  ...
  mapLegacyConfigToEnv(legacyConfig) {
    ...
    if (legacyConfig.streamQuality !== undefined) {
      const dimensions = this.mapQualityToStreamDimensions(
        legacyConfig.streamQuality
      );
  ...
  mapExtendedConfigToEnv(extendedConfig) {
    ...
    if (extendedConfig.cleanupOlderThanDays !== undefined) {
      envUpdates.CLEANUP_OLDER_THAN_DAYS =
        extendedConfig.cleanupOlderThanDays.toString();
    }
    if (extendedConfig.autoGenerateVideo !== undefined) {
      envUpdates.AUTO_GENERATE_VIDEO =
        extendedConfig.autoGenerateVideo.toString();
    }
    if (extendedConfig.enableHardwareAcceleration !== undefined) {
      envUpdates.ENABLE_HARDWARE_ACCELERATION =
        extendedConfig.enableHardwareAcceleration.toString();
    }
    ...
    return {
      imageQuality: [
        ...
        },
        {
          value: "high",
          label: "High (4K)",
          description: "3840x2160 resolution",
        },
      ],
      streamFps: [
        ...
        { value: 25, label: "25 FPS", description: "High quality" },
        {
          value: 30,
          label: "30 FPS",
          description: "Maximum smoothness, high CPU",
        },
      ],
      streamQuality: [
        {
          value: "low",
          label: "Low (480p)",
          description: "640x480 - Low bandwidth",
        },
        {
          value: "medium",
          label: "Medium (720p)",
          description: "1280x720 - Balanced",
        },
        {
          value: "high",
          label: "High (1080p)",
          description: "1920x1080 - High quality",
        },
      ],
      videoFps: [
      ...
      cameraType: [
        {
          ...
          description: "RPi Camera Module v2/v3",
        },
        {
          value: "usb",
          label: "USB Camera",
        ...
        },
        {
          ...
          description: "Any V4L2 compatible camera",
        },
      ],
      rotation: [
      ...
      logLevel: [
        ...
        { value: "debug", label: "Debug", description: "Detailed debugging" },
        {
          value: "trace",
          label: "Trace",
          description: "Very detailed logging",
        },
      ],
    };
  }
